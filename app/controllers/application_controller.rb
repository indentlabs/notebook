...
class ApplicationController < ActionController::Base
  ...
  def cache_current_user_content
    ...
    if @universe_scope
      @current_user_content['Timeline'] = current_user.timelines.includes(:image_uploads).where(universe_id: @universe_scope.try(:id)).to_a
      @current_user_content['Document'] = current_user.documents.includes(:image_uploads).unarchived.where(universe_id: @universe_scope.try(:id)).order('updated_at DESC').to_a
      @current_user_content['Book'] = current_user.books.includes(:image_uploads).unarchived.where(universe_id: @universe_scope.try(:id)).order('updated_at DESC').to_a
    else
      @current_user_content['Timeline'] = current_user.timelines.includes(:image_uploads).to_a
      @current_user_content['Document'] = current_user.documents.includes(:image_uploads).unarchived.order('updated_at DESC').to_a
      @current_user_content['Book'] = current_user.books.includes(:image_uploads).unarchived.order('updated_at DESC').to_a
    end
  end
