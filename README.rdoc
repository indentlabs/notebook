...
==== Google
- Sign into https://console.developers.google.com/
- Create a new project: "notebook"
- Credentials > OAuth Consent Screen
-- Fill out the product name (at least)
-- Save
- Credentials > Create Credentials > Oauth Client ID
-- Application Type: Web Application
-- Name: Notebook.ai
-- Add authorized javascript origins. For localhost, add `http://localhost:3000`
-- Add authorized redirect URI. For localhost, add `http://localhost:3000/users/auth/google_oauth2/callback`
- Add the Google+ API
-- Library > Search "Google+ API" > Enable
- Add the resulting credentials to .env
- Probably wait ~1-2 mins for changes to take effect on Google's end
